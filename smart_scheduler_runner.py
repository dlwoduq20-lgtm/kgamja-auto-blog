...
def run_dual_publish():
    """
    Run both Korean and Japanese auto-posting pipelines.
    """
    print("\n🇰🇷 [한국어 블로그 발행 시작]")
    ...
    return res_kr.returncode == 0 or res_jp.returncode == 0
...
def main():
    ...
    print(f"🕒 [스마트 한·일 듀얼 스케줄러] 현재 시각(KST/JST): {kst_now.strftime('%Y-%m-%d %H:%M:%S')}")
    
    history = load_history()
    today_history = history.get(today_str, [])

    if force_run:
        print("⚡ [수동 실행 감지] 한국 & 일본 블로그 동시 발행을 시작합니다.")
        success = run_dual_publish()
        if success:
    ...
    if current_hour in TARGET_HOURS:
        ...
        if hour_key not in today_history:
            print(f"🎯 [발행 시점 도달] 오늘 {current_hour}시 글 발행을 시작합니다 (한·일 동시)!")
            success = run_dual_publish()
            if success:
                today_history.append(hour_key)
                history[today_str] = today_history
                save_history(history)
                print(f"✅ {current_hour}시 한국 & 일본 블로그 동시 발행 완료 및 기록 저장 성공!")
                sys.exit(0)
            else:
